...
async function buildGuideCanonicalRedirects() {
  try {
    ...
    for (const row of rows || []) {
      ...
      const canonicalSlug = resolveGuideCanonicalSlug(slug, id);
      if (!canonicalSlug || canonicalSlug === id) continue;

      const canonicalPath = `/guide/${canonicalSlug}`;

      // UUID 상세 URL은 색인 대상이 아니므로 정식 SEO URL로 보냅니다.
      if (UUID_PATTERN.test(id)) {
        addRedirect(redirects, seen, `/guide/${id}`, canonicalPath);
      }

      // DB slug가 아직 hex suffix 없는 구형 slug라면 정식 slug-id8 URL로 보냅니다.
      // 예: /guide/does-changing-name-change-luck → /guide/does-changing-name-change-luck-0f7858ac
      if (slug && slug !== canonicalSlug) {
        addRedirect(redirects, seen, `/guide/${slug}`, canonicalPath);
      }
    }
...
const headers = [{ src: '^/yearly-fortune/.+$', headers: { 'X-Robots-Tag': 'noindex, follow' }, continue: true }, { src: '/assets/(.*)', headers: { 'Cache-Control': 'public, max-age=31536000, immutable' }, continue: true }, { src: '/(.*)\\.js', headers: { 'Cache-Control': 'public, max-age=3600, s-maxage=3600' }, continue: true }, { src: '/(.*)\\.css', headers: { 'Cache-Control': 'public, max-age=3600, s-maxage=3600' }, continue: true }, { src: '/(.*)\\.(?:webp|jpg|jpeg|png|gif|svg|ico)', headers: { 'Cache-Control': 'public, max-age=86400, s-maxage=86400' }, continue: true }, { src: '/sitemap(?:-[\\w-]+)?\\.xml', headers: { 'Cache-Control': 'public, max-age=0, s-maxage=3600, must-revalidate' }, continue: true }, { src: '/(.*)\\.html', headers: { 'Cache-Control': 'public, max-age=3600, s-maxage=3600' }, continue: true }];
async function main() { const redirectRoutes = dedupeRoutes([...extraRedirects, ...loadRedirectsFromVercelJson(), ...(await buildGuideCanonicalRedirects()), ...buildDeletedUrlRedirects()]); const config = { version: 3, routes: [...redirectRoutes, ...headers, { handle: 'filesystem' }, { src: '^/yearly-fortune/.+$', dest: '/yearly-fortune-detail/index.html' }, { src: '^/dream/.+$', status: 404, dest: '/404.html' }, { src: '/.*', dest: '/index.html' }] }; fs.writeFileSync(path.join(outDir, 'config.json'), JSON.stringify(config, null, 2)); console.log(`✅ Generated .vercel/output/config.json with ${redirectRoutes.length} redirects`); console.log('🎉 Vercel Build Output API structure ready!'); }
