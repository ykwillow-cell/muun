...
async function main() { const redirectRoutes = dedupeRoutes([...extraRedirects, ...loadRedirectsFromVercelJson(), ...(await buildGuideCanonicalRedirects()), ...buildDeletedUrlRedirects()]); const config = { version: 3, routes: [...redirectRoutes, ...headers, { src: '^/yearly-fortune/.+$', dest: '/yearly-fortune-detail' }, { handle: 'filesystem' }, { src: '^/dream/.+$', status: 404, dest: '/404.html' }, { src: '/.*', dest: '/index.html' }] }; fs.writeFileSync(path.join(outDir, 'config.json'), JSON.stringify(config, null, 2)); console.log(`✅ Generated .vercel/output/config.json with ${redirectRoutes.length} redirects`); console.log('🎉 Vercel Build Output API structure ready!'); }
