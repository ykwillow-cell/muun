...
function getFallbackCategoryEntries(currentEntry: DictionaryIndexItem | undefined, maxItems: number) {
  ...
  return DICTIONARY_INDEX.filter((entry) => entry.id !== currentEntry.id && entry.category === currentEntry.category).slice(0, maxItems);
}

export function RelatedTermsSection({ currentTermId, currentTags = [], maxItems = 5 }: RelatedTermsSectionProps) {
  const relatedTerms = useMemo(() => {
    if (!DICTIONARY_INDEX.length) return [];
    const currentEntry = DICTIONARY_INDEX.find((entry) => entry.id === currentTermId);
    if (!currentEntry) return [];
    if (!currentTags.length) return getFallbackCategoryEntries(currentEntry, maxItems);

    const related = DICTIONARY_INDEX
      .filter((entry) => entry.id !== currentTermId)
      .map((entry) => ({ entry, matchScore: (entry.tags || []).filter((tag) => currentTags.includes(tag)).length }))
