...
const SERVICES: Array<{ href: string; label: string; desc: string; badge?: string; tone: string; Icon: LucideIcon }> = [
  { href: '/daily-fortune', label: '오늘의 운세', desc: '하루 운세 체크', tone: 'cp', Icon: Clock3 },
  { href: '/manselyeok', label: '만세력', desc: '사주 네 기둥 확인', tone: 'cm', Icon: Calendar },
  { href: '/tojeong', label: '토정비결', desc: '월별 흐름 보기', tone: 'cb', Icon: Sparkles },
  { href: '/astrology', label: '점성술', desc: '네이탈 차트 분석', tone: 'cc', Icon: Star, badge: '인기' },
  { href: '/tarot', label: '타로', desc: '질문 중심 리딩', tone: 'cv', Icon: Layers3 },
  { href: '/family-saju', label: '가족사주', desc: '가족 오행 조화', tone: 'cpk', Icon: Users },
  { href: '/hybrid-compatibility', label: '사주×MBTI', desc: '성향 결합 궁합', tone: 'ct', Icon: Brain, badge: 'NEW' },
  { href: '/dream', label: '꿈해몽', desc: '자주 찾는 상징 해석', tone: 'cam', Icon: Moon },
  { href: '/naming', label: '작명소', desc: '이름 후보 비교', tone: 'cg', Icon: PenLine, badge: 'NEW' },
  ...
  { href: '/past-life', label: '전생 보기', desc: '전생 이미지 탐색', tone: 'cp', Icon: Globe2 },
];
    ...
    <div className="mu-subpage-screen min-h-screen bg-background text-foreground pb-20 antialiased">
      ...
      <main className="mu-service-main px-4 py-6">
        <section className="mu-more-page">
          ...
          <div className="svgrid mu-home-html-services-grid mu-more-page__grid--wide">
            {SERVICES.map(({ href, label, desc, badge, tone, Icon }) => (
              <Link key={href} href={href} className={`sc ${tone}`}>
                ...
                <span className="mu-more-mini-link">바로가기 <ArrowRight size={13} /></span>
              </Link>
            ))}
          </div>
        </section>
