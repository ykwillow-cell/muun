...
export function getDayPillar(date: Date): { stem: HeavenlyStem, branch: EarthlyBranch } {
  // 타임존 문제를 방지하기 위해 로컬 날짜의 00:00:00 기준 정수 날짜 차이만 계산
  // JavaScript의 Date 객체는 타임존에 따라 date.getTime() 값이 달라질 수 있음.
  // 특히 Vercel 같은 서버 환경(UTC)과 사용자 환경(KST) 간의 차이로 인해 1일 오차가 발생 가능.
  
  // 기준일: 2000년 1월 1일 戊午(무오)일
  const baseYear = 2000;
  const baseMonth = 0; // 1월
  const baseDay = 1;
  const baseStemIdx = 4; // 戊
  const baseBranchIdx = 6; // 午
  
  // UTC 기준 정시(00:00:00)로 날짜 차이 계산하여 타임존 오차 제거
...
export function calculateSaju(birthDate: Date, gender: 'male' | 'female'): SajuResult {
  if (!birthDate || isNaN(birthDate.getTime())) {
    console.error("Invalid birthDate passed to calculateSaju:", birthDate);
