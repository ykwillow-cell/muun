  ...
  const metaData: Record<string, { title: string, description: string, h1?: string, services?: { href: string, label: string }[], bodyContent?: string }> = {
    ...
    '/daily-fortune': {
      ...
      bodyContent: `
        <section>
          <h2>오늘의 운세란?</h2>
          <p>오늘의 운세는 사주팔자(四柱八字)와 오늘의 일진(日辰)을 결합하여 개인별 맞춤 하루 운세를 제공합니다. 매일 변화하는 천간(天干)과 지지(地支)의 기운이 나의 사주와 어떻게 상호작용하는지 분석하여, 오늘 하루의 길흉(吉凶)과 주의사항을 안내합니다.</p>
    ...
    '/hybrid-compatibility': {
      ...
      bodyContent: `
        <section>
          <h2>사주×MBTI 하이브리드 궁합이란?</h2>
          <p>사주×MBTI 하이브리드 궁합은 동양의 사주 오행(五行) 분석과 서양의 MBTI 성격 유형 이론을 결합한 무운만의 독창적인 궁합 분석 방법입니다. 사주가 타고난 에너지의 '하드웨어'라면, MBTI는 후천적으로 형성된 성격의 '소프트웨어'입니다. 두 가지를 함께 분석하면 훨씬 더 입체적이고 정확한 궁합 결과를 얻을 수 있습니다.</p>
    ...
    '/family-saju': {
      title: "가족사주 분석 - 우리 가족 궁합과 오행 조화 확인 | 무운 (MuUn)",
      description: "가족사주(家族四柱)는 가족 구성원 각각의 사주팔자(四柱八字)를 종합적으로 분석하여, 가족 간의 오행(五行) 조화와 관계 역학을 파악하는 명리학의 응용 분야입니다.",
      h1: "가족사주 분석",
      bodyContent: `
        <section>
          <h2>가족사주(家族四柱)란?</h2>
          <p>가족사주(家族四柱)는 가족 구성원 각각의 사주팔자(四柱八字)를 종합적으로 분석하여, 가족 간의 오행(五行) 조화와 관계 역학을 파악하는 명리학(命理學)의 응용 분야입니다. 개인의 사주를 넘어 가족 전체의 기운이 어떻게 상호작용하는지를 분석함으로써, 가족 관계의 강점과 보완해야 할 부분을 파악할 수 있습니다.</p>
    ...
    '/guide': {
      ...
      bodyContent: `
        <section>
          <h2>무운 운세 칼럼이란?</h2>
          <p>무운 운세 칼럼은 30년 내공의 역술인이 직접 집필한 사주 명리학 전문 칼럼 시리즈입니다. 어렵고 딱딱하게 느껴지는 사주 이론을 실생활에 바로 적용할 수 있는 쉽고 실용적인 내용으로 풀어냅니다.</p>
          <h2>칼럼 주요 주제</h2>
          <ul>
